...
var reportController = function (reportService, querystring) {
    ...
    var onReportSuccess = function (results, query, res) {
        ...
        res.render('reportView', {
            ...
            entryStartDate: moment(query.from).format('YYYY-MM-DD'),
            entryEndDate: moment(query.to).format('YYYY-MM-DD'),
            projectName: query.projectName,
            projectStatus:query.projectStatus,
            projectToken: query.projectToken.slice(6, query.projectToken.length),
            projectUser: query.projectUser,
            isHidden: 'block',
    ...
    var getReport = function (req, res) {
        ...
        let queryParamStr = {
            ...
            projectStatus: query.projectStatus,
            projectToken: userInfo['token'],
            projectUser: userInfo['username'],
        ...
        if (!userInfo.username) {
            res.render('reportView', {
                entryStartDate: moment(queryParamStr.today).subtract(14, 'd').format('YYYY-MM-DD'),
                entryEndDate: moment(queryParamStr.today).add(1, 'd').format('YYYY-MM-DD'),
                entryCurrentDate: moment(queryParamStr.today).format('YYYY-MM-DD'),
                isHidden: 'none'
            });
            return;
        }
